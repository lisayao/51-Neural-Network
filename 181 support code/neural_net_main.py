...
def main():
  ...
  print '* * * * * * * * *'
  # Train the network.  Adding networkType for hidden/convolutional.
  network.Train(images, validation, rate, epochs, networkType)

  DataReader.DumpWeights(network.network.weights, 'weights')


  # network.network.weights = DataReader.ReadWeights('weights')  
