...
class Node:
  ...
  def AddConvolutionalInput(self, nodes, weights, network):
    self.inputs = nodes
...
class NeuralNetwork:
  ...
  def AddNode(self, node, node_type):
    ...
    if node_type == self.INPUT:
      self.inputs.append(node)
    elif node_type == self.LAYER1:
      self.layer1_nodes.append(node)
    elif node_type == self.LAYER2:
      self.layer2_nodes.append(node)
...
class NetworkFramework(object):
  ...
  def Classify(self, image, network_type):
    if network_type == "UI":
      self.FeedForwardFn(self.network, image, network_type)
    else: 
      input = self.Convert(image)
      self.FeedForwardFn(self.network, input, network_type)
    return self.GetNetworkLabel()

  def Performance(self, images, network_type):

    # Loop over the set of images and count the number correct.
    correct = 0
    for image in images:
      if self.Classify(image, network_type) == image.label:
        correct += 1
    return correct * 1.0 / len(images)

  def Train(self, images, validation_images, learning_rate, epochs, network_type):
    ...
    performance_log = []
    performance_log.append((self.Performance(images, network_type), \
      self.Performance(validation_images, network_type)))
    
    # creating file pointer for performance log
    outfile = open("performance_log1", 'a')

    # Loop through the specified number of training epochs.
    for i in range(epochs):

      # This calls our function in neural_net_impl.py.
      self.TrainFn(self.network, inputs, targets, learning_rate, 1, network_type)
      ...
      outfile.write('%d,%.3f,%.8f,%.3f \n' % (i + 1, learning_rate, perf_train, perf_validate))
